...
class Store:
    def __init__(self, path: str):
        # One connection shared between the BLE scanner (event-loop thread) and
        # FastAPI's threadpool workers — serialize access ourselves.
    ...
    def aggregated_series(self, start: int, end: int, bucket: int) -> list[dict]:
        ...
        read = {r[0]: r[1:] for r in read_rows}
        hist = {h[0]: h[1:] for h in hist_rows}
        hist_b = {}
        if bucket >= 3600:
